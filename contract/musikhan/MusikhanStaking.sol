...
contract MusikhanStaking is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public constant hanTokenPerLpToken = 2314814815; // 0.000000002314814815
    uint256 public tokenQuota = 5000000 ether; // 365000000000000000000000 컨트랙랙트에 입금
    ...
    struct Staker {
        ...
        string symbol;
        address l2Ca;
        uint256 amountStaked; // 스테이킹한 금액
        uint256 claimedReward; // 지금까지 받은 보상
        uint256 timeOfLastUpdate; // 마지막으로 스테이킹한 시간
        uint256 unclaimedRewards; // 클레임 받지 않고 쌓인 보상
    }
    mapping(address => mapping(address => Staker)) private stakers;
    mapping(address => uint256) public totalReward; // 지금까지 받은 총 보상
    ...
    mapping(address => mapping(address => uint256)) private stakerAmount; // 토큰 주소 => 내 지갑 주소 => 금액
    ...
    function transferRewardToken(uint256 _amount) public onlyOwner {
    ...
    function setTokenQuota(uint256 _newQuota) public onlyOwner {
        require(_newQuota > totalSupply, "Too Small Quota");
        tokenQuota = _newQuota;
    }

    // 입력한 토큰에 대한 유저가 가진 정보
    function getStaker(address _l2Ca, address _user)
        public
        view
    ...
    function getStakingTokenList(address _user)
        public
        view
    ...
    function getClaimTokenList(address _user)
        public
        view
        returns (address[] memory)
    {
    ...
    function getStakerAddress(address _l2Ca)
        public
        view
        returns (address[] memory)
    {
    ...
    function getStakerAmount(address _l2Ca, address _user)
        public
        view
        returns (uint256)
    {
    ...
    // Staking function
    function stake(address _l2Ca, uint256 _amount)
        public
        nonReentrant
        whenNotPaused
    {
        require(
            rewardToken.balanceOf(address(this)) -
                ...
                rewardsDuration * ((_amount * hanTokenPerLpToken) / 10**18),
        ...
        if (staker.amountStaked > 0) {
            ...
            staker.unclaimedRewards +=
                stakedTime *
                ((staker.amountStaked * hanTokenPerLpToken) / 10**18);
    ...
    function claimReward(address _l2Ca) public nonReentrant {
        ...
        require(
            staker.unclaimedRewards < rewardToken.balanceOf(address(this)),
            "Short Reward"
        );
        uint256 stakedTime = block.timestamp - staker.timeOfLastUpdate;
        staker.unclaimedRewards +=
    ...
    // 스테이킹한 정보 저장하는 함수수
    function setStaker(address _l2Ca, uint256 _amount) internal {
        IMusikhan stakingToken = IMusikhan(_l2Ca);
        Staker storage staker = stakers[_l2Ca][msg.sender];
    ...
    function checkMintToken(address _l2Ca, address _user) internal {
        for (uint256 i = 0; i < musikhan.getMyTokenList(_user).length; i++) {
    ...
    function findL2TokenList(address _l2Ca) internal {
        for (uint256 i = 0; i <= musikhan.getL2TokenList().length - 1; i++) {
