...
contract Musikhan is Ownable {
    ...
    bool private check;
    uint256 private defaultTime = 120; //20분
    ...
    mapping(address => uint256) private lostTokenAmount;
    ...
    function getExistedTokenInfo(address _user)
        public
        view
        returns (ExistedTokenInfo memory)
    {
    ...
    function getL1TokenInfo(address _user)
        public
        view
        returns (L1TokenInfo memory)
    {
        return l1Tokens[_user];
    }

    // 토큰 주소 입력시 L2TokenInfo 출력 함수
    function getL2TokenInfo(address _l2Ca)
        public
        view
        returns (L2TokenInfo memory)
    {
        return l2Tokens[_l2Ca];
    }

    // 주소 입력시 내가 민팅한 토큰 리스트 출력
    function getMyTokenList(address _user)
        public
        view
    ...
    function getLostTokenAmount(address _l2Ca) public view returns (uint256) {
    ...
    function setL1TokenData(bytes memory _data) public {
        ...
        (
            ...
            l1Token.amount,
            l1Token.l1Ca,
            l1Token.receivedTime
        ) = abi.decode(_data, (string, string, uint256, address, uint256));
        emit L1TokenData(
            l1Token.name,
            l1Token.symbol,
            l1Token.amount,
    ...
    function setL2TokenData(address _l2Ca, bytes memory _data) public {
        ...
        (
            ...
            l2Token.l2Ca,
            l2Token.owner
        ) = abi.decode(_data, (string, string, address, address, address));
        emit BridgeL2TokenData(
            l2Token.name,
            l2Token.symbol,
            l2Token.l1Ca,
            l2Token.l2Ca,
    ...
    function setNewL2TokenData(address _l2Ca, bytes memory _data) public {
        ...
        (
            ...
            l2Token.owner,
            l2Token.root
        ) = abi.decode(_data, (string, string, address, address, bytes32));
        emit NewL2TokenData(
            l2Token.name,
            l2Token.symbol,
            l2Token.l2Ca,
    ...
    function setExistedTokenData(address _user, bytes memory _data) public {
        ...
        (
            ...
            existedToken.symbol,
            existedToken.amount,
            existedToken.existedTokenCa,
            existedToken.swapTokenCa
        ) = abi.decode(_data, (string, string, uint256, address, address));
        emit ExistedTokenData(
            existedToken.name,
            existedToken.symbol,
            existedToken.amount,
            existedToken.existedTokenCa,
    ...
    function setL2SwapTokenData(address _l2SwapToken, bytes memory _data)
        public
    {
    ...
    function mint(address _l2Ca) public {
        ...
        require(
            block.timestamp >= (l1Token.receivedTime + defaultTime),
            "Need more time"
        );
    ...
    function mintL2SwapToken(address _l2Ca, address _user) public {
        ...
        L2TokenInfo storage l2Token = l2Tokens[_l2Ca];
        require(
            existedToken.existedTokenCa == l2Token.existedToken,
            "Token do not match"
        );
    ...
    function findL2MintTokenList(address _ca) internal {
        for (uint256 i = 0; i < myTokenList[msg.sender].length; i++) {
    ...
    function findL1TokenList(address _ca) internal {
        for (uint256 i = 0; i <= l1TokenList.length - 1; i++) {
    ...
    function findL2TokenList(address _ca) internal {
        for (uint256 i = 0; i <= l2TokenList.length - 1; i++) {
    ...
    event L2SwapTokenData(
        ...
        string symbol,
        address indexed l2TokenAddress,
        address indexed swapTokenAddress,
        address indexed owner
    );
    event ExistedTokenData(
        string name,
        string symbol,
        uint256 amount,
        address indexed existedTokenAddress,
        address indexed swapTokenAddress
    );
    event MintBridgeL2Token(
        uint256 amount,
        address indexed l2Ca,
        address indexed user
    );
    event MintNewL2Token(
        uint256 amount,
        address indexed l2Ca,
        address indexed user
    );
    event LostMint(uint256 amount, address indexed l2Ca, address indexed user);
}
