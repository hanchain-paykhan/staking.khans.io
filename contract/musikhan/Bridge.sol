...
contract Bridge is Ownable {
    ...
    function sendToken(address _l1Ca, address _l2Ca) public {
        ...
        crossDomainMessenger.sendMessage(
            musikhan,
            abi.encodeWithSignature("setUser(address)", msg.sender),
            1000000
        );
        crossDomainMessenger.sendMessage(
            musikhan,
            abi.encodeWithSignature("setL1TokenData(bytes)", tokenData),
            1000000
        );
        crossDomainMessenger.sendMessage(
            musikhan,
            abi.encodeWithSignature("setL1TokenAddress(bytes)", tokenAddress),
            1000000
        );
    ...
    event L1TokenData(
        ...
        address indexed l1TokenCa,
        address indexed l2TokenCa,
        uint256 amount
    );
