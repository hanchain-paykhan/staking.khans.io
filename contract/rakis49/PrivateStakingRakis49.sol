...
contract PrivateStakingRakis49 is ReentrancyGuard, Ownable, Pausable {
    ...
    struct Staker {
        ...
        uint256 totalRewardReceived;   // The total amount of HANeP Token received as reward
        uint256 unclaimedReward;  // The accumulated amount of HANeP token before "CLAIM"
    ...
    function stake(uint32 _participationCode, uint256 _amount) public nonReentrant whenNotPaused {
        ...
        require(rewardToken.balanceOf(address(this)) - (((withdrawDuration * (totalSupply * hanepTokenPerLpToken)) / 10**18)) > withdrawDuration * ((_amount * hanepTokenPerLpToken) / 10**18), "Total amount of rewards is too high");
    ...
    function claimReward() public nonReentrant {
        ...
        for(uint i = 0; i < stakerArray[msg.sender].length; i++) {
            reward += (block.timestamp - stakerArray[msg.sender][i].timeStarted) * ((stakerArray[msg.sender][i].amount * hanepTokenPerLpToken) / 10**18);
    ...
    function rewardView(address _user) public view returns(uint256) {
        ...
        for(uint i = 0; i < stakerArray[_user].length; i++) {
            reward += (block.timestamp - stakerArray[_user][i].timeStarted) * ((stakerArray[_user][i].amount * hanepTokenPerLpToken) / 10**18);
        }
        return reward;
    }
