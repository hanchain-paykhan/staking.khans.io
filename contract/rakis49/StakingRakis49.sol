...
contract StakingRakis49 is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public constant hanepTokenPerLpToken = 163665921396; // Quantity of HANeP tokens rewarded per LP token
    ...
    uint256 public rewardsDuration = 365 days; // The total amount of HANeP token available for reward = hanepTokenPerLpToken * tokenVolume * rewardsDuration
    ...
    mapping(address => uint256) public getRewardReleased; // The total amount of HANeP Token received as reward
    mapping(address => uint256) public getBalance; // The accumulated amount of HANeP token before "CLAIM"
    ...
    function stake(uint256 amount) public nonReentrant whenNotPaused {
        ...
        require(
            rewardToken.balanceOf(address(this)) -
                (
                    ((rewardsDuration * (totalSupply * hanepTokenPerLpToken)) /
                        10**18)
                ) >
                rewardsDuration * ((amount * hanepTokenPerLpToken) / 10**18),
        ...
        if (getAmount[msg.sender] > 0) {
            ...
            getBalance[msg.sender] +=
                stakedTime *
                ((getAmount[msg.sender] * hanepTokenPerLpToken) / 10**18);
    ...
    function updateReward() public view returns (uint256) {
        uint256 value;
        value += ((getAmount[msg.sender] * hanepTokenPerLpToken) / 10**18);
        return value;
    }
